...
class PersistentQueue<T>(private val options: QueueOptions<T>) {
    private val logger = LoggerFactory.getLogger(javaClass)
    private val scope = CoroutineScope(Dispatchers.IO)
    private val act = scope.queueMessageHandler(options)

    private inline fun logSentry(name: String, operation: String, block: () -> Unit) {
        sentry(name, operation) {
            it.setTag("parallel", options.queueWriteLayer.concurrentPushPop.toString())
            block()
        }
    }
