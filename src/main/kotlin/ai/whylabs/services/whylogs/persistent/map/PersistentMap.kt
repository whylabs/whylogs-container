...
class PersistentMap<K, V>(writer: MapWriteLayer<K, V>) : AutoCloseable {
    ...
    private val act =
        mapMessageHandler(MapMessageHandlerOptions(CoroutineScope(newSingleThreadContext("persistent map")), writer))
     ...
     */
    suspend fun set(key: K, block: (V?) -> V?) {
        val done = CompletableDeferred<V?>()
     ...
     */
    suspend fun reset(block: (Map<K, V>) -> Map<K, V>) {
        val done = CompletableDeferred<V?>()
